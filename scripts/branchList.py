...
table_name = 'api_listaFiliais'
url = f"{COMPLEO_API_BASE_URL}/branchlist/{COMPLEO_COMPANY_ID}"
    ...
    try:
        ...
        if is_first_page:
            try:
                ...
                try:
                    index_query = f"CREATE INDEX idx_branch_id ON {table_name} (id)"
        ...
        if isinstance(data, dict) and 'data' in data and isinstance(data['data'], list):
            ...
            branches = data
            logging.info(f"Lista direta de {len(branches)} filiais recebida")
        else:
        ...
        for branch in branches:
            ...
            branch_id = safe_value(branch.get('id'))
            
            if branch_id in processed_ids:
                count_skipped += 1
                continue
            ...
            try:
                ...
                logging.warning(f"Erro ao verificar duplicidade para filial {branch_id}: {e}")
            ...
            try:
                cursor.execute(insert_query, (
                    branch_id,
                    safe_value(branch.get('name')),
                    safe_value(branch.get('createdAt')),
                    safe_value(branch.get('lastUpdatedAt'))
                ))
                ...
                logging.error(f"Erro ao inserir filial {branch_id}: {e}")
                try:
                    logging.error(f"Filial problemática: {json.dumps(branch, ensure_ascii=False)[:500]}")
                except:
                    logging.error("Não foi possível serializar a filial problemática")
                continue
        ...
        error_message = str(e)
        logging.error(f"Erro ao salvar dados de filiais: {error_type} - {error_message}")
        try:
            cursor.rollback()
        except:
...
def process_filiais(session, cursor, json_dir, processed_ids):
    logging.info("Iniciando processamento de filiais")
    ...
    if success:
        ...
        logging.info(f"Total de {total_records} filiais, {total_pages} páginas")
        ...
        branches_page1 = data.get('data', [])
        branches_count = len(branches_page1)
        for branch in branches_page1:
            if branch.get('id'):
                total_processed += 1
        

        
        has_more_pages = branches_count > 0 and current_page < total_pages
        ...
        try:
            ...
            if duplicates_count > 0:
                logging.warning(f"Verificação final: {duplicates_count} IDs de filiais duplicados encontrados")
        ...
        try:
            ...
            logging.info(f"Processamento de filiais concluído: {total_count} registros inseridos ({unique_codes} IDs únicos) em {current_page} páginas")
        ...
        error_message = f"Erro na requisição inicial de filiais: {data}"
        logging.error(error_message)
        return 0



def process_filiais_compleo():
    ...
    try:
        ...
        json_dir = os.path.join(os.path.dirname(script_dir), "Jsons", "listaFiliais")
        ...
        with requests.Session() as session:
            ...
            total_processed = process_filiais(
                session=session,
                cursor=cursor,
                json_dir=json_dir,
            ...
            try:
                ...
                duplicate_info = ""
                if duplicates:
                    duplicate_info = f"\nATENÇÃO: Encontrados {len(duplicates)} IDs de filiais duplicados"
                ...
                if duplicates:
                    logging.warning(f"Atenção: {len(duplicates)} IDs de filiais duplicados encontrados")
            except Exception as e:
        ...
        error_message = (f"Erro geral no processamento: {e} "
                         f"Arquivo: listaFiliais.py, Hora: {error_time}, "
...
if __name__ == "__main__":
    print("Iniciando processamento de filiais da Compleo...")
    total_inseridos = process_filiais_compleo()
    print(f"Processamento concluído! {total_inseridos} filiais inseridas.")
