...
table_name = 'api_listaContatos'
url = f"{COMPLEO_API_BASE_URL}/contactlist/{COMPLEO_COMPANY_ID}"
...
def save_to_database(data, table_name, cursor, is_first_page=True, processed_ids=None):
    try:
        ...
        if is_first_page:
            try:
                ...
                CREATE TABLE {table_name} (
                    ...
                    createdByUser NVARCHAR(500),
                    lastUpdatedByUser NVARCHAR(500),
                    total INT,
                    totalFiltered INT,
                ...
                try:
                    index_query = f"CREATE INDEX idx_contact_id ON {table_name} (id)"
        ...
        if isinstance(data, dict) and 'data' in data and isinstance(data['data'], list):
            ...
            contacts = data
            logging.info(f"Lista direta de {len(contacts)} contatos recebida")
        else:
        ...
        for contact in contacts:
            ...
            contact_id = safe_value(contact.get('id'))
            ...
            try:
                ...
                logging.warning(f"Erro ao verificar duplicidade para contato {contact_id}: {e}")
            ...
            try:
                ...
                cursor.execute(insert_query, (
                    ...
                    'sim' if contact.get('accessEnabled') else 'não',
                    json.dumps([c.get('name', '') if isinstance(c, dict) else c for c in contact.get('customers', [])], ensure_ascii=False) if isinstance(contact.get('customers'), list) else safe_value(contact.get('customers')),
                    safe_value(contact.get('createdAt')),
                    safe_value(contact.get('lastUpdatedAt')),
                    contact.get('createdByUser', {}).get('name') if isinstance(contact.get('createdByUser'), dict) else safe_value(contact.get('createdByUser')),
                    contact.get('lastUpdatedByUser', {}).get('name') if isinstance(contact.get('lastUpdatedByUser'), dict) else safe_value(contact.get('lastUpdatedByUser')),
                    total_value,
                ...
                logging.error(f"Erro ao inserir contato {contact_id}: {e}")
                try:
                    logging.error(f"Contato problemático: {json.dumps(contact, ensure_ascii=False)[:500]}")
                except:
                    logging.error("Não foi possível serializar o contato problemático")
                continue
        ...
        error_message = str(e)
        logging.error(f"Erro ao salvar dados de contatos: {error_type} - {error_message}")
        try:
            cursor.rollback()
        except:
...
def process_contatos(session, cursor, json_dir, processed_ids):
    logging.info("Iniciando processamento de contatos")
    ...
    if success:
        ...
        logging.info(f"Total de {total_records} contatos, {total_pages} páginas")
        ...
        contacts_page1 = data.get('data', [])
        contacts_count = len(contacts_page1)
        for contact in contacts_page1:
            if contact.get('id'):
                total_processed += 1
        

        
        has_more_pages = contacts_count > 0 and current_page < total_pages
        ...
        try:
            ...
            if duplicates_count > 0:
                logging.warning(f"Verificação final: {duplicates_count} IDs de contatos duplicados encontrados")
        ...
        try:
            ...
            logging.info(f"Processamento de contatos concluído: {total_count} registros inseridos ({unique_codes} IDs únicos) em {current_page} páginas")
        ...
        error_message = f"Erro na requisição inicial de contatos: {data}"
        logging.error(error_message)
        return 0



def process_contatos_compleo():
    ...
    try:
        ...
        json_dir = os.path.join(os.path.dirname(script_dir), "Jsons", "listaContatos")
        ...
        with requests.Session() as session:
            ...
            total_processed = process_contatos(
                session=session,
                cursor=cursor,
                json_dir=json_dir,
            ...
            try:
                ...
                duplicate_info = ""
                if duplicates:
                    duplicate_info = f"\nATENÇÃO: Encontrados {len(duplicates)} IDs de contatos duplicados"
                ...
                if duplicates:
                    logging.warning(f"Atenção: {len(duplicates)} IDs de contatos duplicados encontrados")
            except Exception as e:
        ...
        error_message = (f"Erro geral no processamento: {e} "
                         f"Arquivo: listaContatos.py, Hora: {error_time}, "
...
if __name__ == "__main__":
    print("Iniciando processamento de contatos da Compleo...")
    total_inseridos = process_contatos_compleo()
    print(f"Processamento concluído! {total_inseridos} contatos inseridos.")
