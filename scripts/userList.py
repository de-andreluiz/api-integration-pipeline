    ...
    try:
        ...
        if is_first_page:
            try:
                ...
                try:
                    index_query = f"CREATE INDEX idx_user_id ON {table_name} (id)"
        ...
        if isinstance(data, dict) and 'data' in data and isinstance(data['data'], list):
            ...
            users = data
            logging.info(f"Lista direta de {len(users)} usuários recebida")
        else:
        ...
        for user in users:
            ...
            user_id = safe_value(user.get('id'))
            ...
            try:
                ...
                logging.warning(f"Erro ao verificar duplicidade para usuário {user_id}: {e}")
            
            role_value = None
            if 'role' in user and user['role']:
                if isinstance(user['role'], dict):
                    role_value = user['role'].get('name')
                else:
                    role_value = safe_value(user['role'])
            ...
            try:
                cursor.execute(insert_query, (
                    user_id,
                    safe_value(user.get('name')),
                    role_value,
                    safe_value(user.get('createdAt')),
                    safe_value(user.get('lastUpdatedAt'))
                ))
                ...
                logging.error(f"Erro ao inserir usuário {user_id}: {e}")
                try:
                    logging.error(f"Usuário problemático: {json.dumps(user, ensure_ascii=False)[:500]}")
                except:
                    logging.error("Não foi possível serializar o usuário problemático")
                continue
        ...
        error_message = str(e)
        logging.error(f"Erro ao salvar dados de usuários: {error_type} - {error_message}")
        try:
            cursor.rollback()
        except:
...
def process_usuarios(session, cursor, json_dir, processed_ids):
    logging.info("Iniciando processamento de usuários")
    ...
    if success:
        ...
        has_more_pages = users_count > 0 and current_page < total_pages
        ...
        try:
            ...
            if duplicates_count > 0:
                logging.warning(f"Verificação final: {duplicates_count} IDs de usuários duplicados encontrados")
        ...
        try:
            ...
            logging.info(f"Processamento de usuários concluído: {total_count} registros inseridos ({unique_codes} IDs únicos) em {current_page} páginas")
        ...
        error_message = f"Erro na requisição inicial de usuários: {data}"
        logging.error(error_message)
        return 0



def process_usuarios_compleo():
    ...
    try:
        ...
        json_dir = os.path.join(os.path.dirname(script_dir), "Jsons", "listaUsuarios")
        ...
        with requests.Session() as session:
            ...
            total_processed = process_usuarios(
                session=session,
                cursor=cursor,
                json_dir=json_dir,
            ...
            try:
                ...
                duplicate_info = ""
                if duplicates:
                    duplicate_info = f"\nATENÇÃO: Encontrados {len(duplicates)} IDs de usuários duplicados"
                ...
                if duplicates:
                    logging.warning(f"Atenção: {len(duplicates)} IDs de usuários duplicados encontrados")
            except Exception as e:
        ...
        error_message = (f"Erro geral no processamento: {e} "
                         f"Arquivo: listaUsuarios.py, Hora: {error_time}, "
...
if __name__ == "__main__":
    print("Iniciando processamento de usuários da Compleo...")
    total_inseridos = process_usuarios_compleo()
    print(f"Processamento concluído! {total_inseridos} usuários inseridos.")
